...
fn main() {
    ...
    let t_a = thread::spawn(move || {
        // for msg in rx1 {
            println!("t_a thread: Received {}", rx1.recv().unwrap());
        // }
    });
    ...
    let t_b = thread::spawn(move || {
        // for msg in rx2 {
            println!("t_b thread: Received {}", rx2.recv().unwrap());
        // }
    });

    // let mut msg_vec: Vec<&'static String> = Vec::new();

    // for i in 1..=90{
    //     msg_vec.push("Hello number ".to_owned() + &i.to_string())
    // }

    for letter in vec!["a", "b", "c", "d", "e", "f"]  {
        tx3.send(letter).unwrap();
    }
